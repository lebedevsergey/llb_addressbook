...
(function ($) {
    ...
    InstantSearch.DEFAULTS = {
        ...
        itemTemplate: '\
                <article class="post">\
                    <h2><a href="/addressee/{{ id }}" target="_blank" rel="noopener noreferrer" >{{ firstname }}&nbsp{{ lastname }}</a></h2>\
                    <p class="post-metadata">\
                       <span class="metadata"><i class="fa fa-phone"></i> {{ phone_number }}</span>\
                       <span class="metadata"><i class="fa fa-envelope-o"></i> {{ email }}</span>\
                       <span class="metadata"><i class="fa fa-calendar"></i> {{ birthdate }}</span>\
                    </p>\
                </article>'
    };
